...
class ASRWorkerThread(QThread):
    ...
    def __init__(self, sample_rate=16000, chunk=2048, buffer_seconds=8,
        ...
        if local_vad_path and os.path.exists(local_vad_path):
             print(f"✅ Worker using LOCAL VAD: {local_vad_path}")
             vad_model_id = local_vad_path
             local_files_only = True
        else:
             print(f"⚠️ Local VAD path not found. Using CLOUD VAD.")
    ...
    def run(self):
        ...
        while self.running:
            ...
            while len(audio_buffer) >= self.vad_chunk_samples:
                ...
                res = self.model_vad.generate(input=current_chunk, cache=self.cache_vad, is_final=False, chunk_size=self.vad_chunk_ms)
                if res and "value" in res[0] and len(res[0]["value"]) > 0:
                    ...
                    if last_vad_beg > -1 and last_vad_end > -1: silence_counter += 1
    ...
    def save_feedback_audio(self, audio_id):
        if audio_id not in self.recognized_audio: return ""
        if not os.path.exists("feedback_audio"): os.makedirs("feedback_audio")
